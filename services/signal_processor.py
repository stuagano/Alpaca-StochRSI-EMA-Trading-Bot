...
class SignalProcessor:
    ...
    def __init__(self, trading_executor: TradingExecutor, config: Dict):
        """
        ...
        self.min_signal_gap_seconds = config.get('min_signal_gap_seconds', 300)  # 5 minutes
        self.require_confirmation = config.get('require_confirmation', True)
        self.confirmation_window = config.get('confirmation_window', 3)  # bars
    ...
    async def should_process_signal(self, signal: TradingSignal) -> bool:
        ...
        try:
            ...
            if signal.strength < self.executor.min_signal_strength:
